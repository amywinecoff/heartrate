...
%subj=103;
input_dir = ('~/Dropbox/BardResearch/ACL_Experiments/BridgetSPROJ');
txt_subj = num2str(subj);
data=dlmread ((strcat(input_dir, '/', txt_subj, '.Baseline.txt')), '\t', 11, 0);
...
baseline=mean(raw_ecg(trial_times));
...
out_dir = [input_dir '/BaselineBPM'];
cd (out_dir)
fname=[txt_subj '_baselineBPM.txt'];

dlmwrite (fname, baseline, '\t');
